...
$checks = @(
    # Credentials
    @{Path="HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"; Name="LmCompatibilityLevel"; Expected=5; PolicyId="WIN-SECURITY-007"; Desc="NTLMv2 only"},
    @{Path="HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\WDigest"; Name="UseLogonCredential"; Expected=0; PolicyId="WIN-SECURITY-006"; Desc="WDigest disabled"},
    @{Path="HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"; Name="RunAsPPL"; Expected=1; PolicyId="WIN-SECURITY-008"; Desc="LSA PPL enabled"},
    @{Path="HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"; Name="RestrictAnonymousSAM"; Expected=1; PolicyId="WIN-SECURITY-009"; Desc="Anonymous SAM enum blocked"},
    @{Path="HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient"; Name="EnableMulticast"; Expected=0; PolicyId="WIN-SECURITY-012"; Desc="LLMNR disabled"},
    # Defender
    ...
    # UAC
    @{Path="HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System"; Name="EnableLUA"; Expected=1; PolicyId="WIN-SECURITY-014"; Desc="UAC Admin Approval Mode on"},
    @{Path="HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System"; Name="ConsentPromptBehaviorAdmin"; Expected=1; PolicyId="WIN-SECURITY-014"; Desc="UAC prompt for credentials"},
    # Logging
    @{Path="HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\Audit"; Name="ProcessCreationIncludeCmdLine_Enabled"; Expected=1; PolicyId="WIN-SECURITY-016"; Desc="Event 4688 cmd line logging"},
    @{Path="HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging"; Name="EnableScriptBlockLogging"; Expected=1; PolicyId="WIN-SECURITY-017"; Desc="PS Script Block Logging on"}
)
...
foreach ($c in $checks) {
    ...
    Write-Host "[$($r.Status.PadRight(7))] $($r.PolicyId.PadRight(20)) $($r.Description)" -ForegroundColor $color
